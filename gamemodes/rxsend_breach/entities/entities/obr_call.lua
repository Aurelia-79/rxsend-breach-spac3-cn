...
function ENT:Use( activator, caller )
  ...
  --[[if caller:GetRoleName() == role.SCI_NegoGOC then
    if timer.TimeLeft("RoundTime") < 280 then
      caller:RXSENDNotify("l:GOC_REINFOR_too_late")
      return
    end
  end  
  if caller:GetRoleName() == role.SCI_NegoGOC then
    if timer.TimeLeft("RoundTime") > 630 then
  ...
  if caller:GetRoleName() == role.SCI_SpyUSA or caller:GetRoleName() == role.UIU_Agent_Information  then
    if self.RXSENDNextUIUHackUse and self.RXSENDNextUIUHackUse > CurTime() then return end
    self.RXSENDNextUIUHackUse = CurTime() + 0.35
    ...
    if caller:GetNWInt("CollectedDocument") == 0 and !self:GetHacking() then
      caller:BrProgressBar("l:spyusa_hacking", 5, "nextoren/gui/icons/hand.png", nil, false, function()
        self:EmitSound("^nextoren/others/monitor/start_hacking.wav")
...
if SERVER then
  local UIU_HACK_USE_DISTANCE_SQR = 22500
  local UIU_HACK_TERMINAL_RADIUS_SQR = 22500

  local function IsUIUHacker( ply )
    return IsValid( ply ) and ply:IsPlayer()
      and ( ply:GetRoleName() == role.SCI_SpyUSA or ply:GetRoleName() == role.UIU_Agent_Information )
  end

  -- The actual terminal is a transparent trigger entity.  Use the map console
  -- hit position as a fallback so the interaction remains available to UIU.
  hook.Add( "KeyPress", "RXSEND:UIUHackTerminalFallback", function( ply, key )
    if key != IN_USE or !IsUIUHacker( ply ) or !ply:Alive() then return end

    local trace = ply:GetEyeTrace()
    if !trace.Hit or ply:EyePos():DistToSqr( trace.HitPos ) > UIU_HACK_USE_DISTANCE_SQR then return end

    for _, terminal in ipairs( ents.FindByClass( "obr_call" ) ) do
      if IsValid( terminal ) and trace.HitPos:DistToSqr( terminal:GetPos() ) <= UIU_HACK_TERMINAL_RADIUS_SQR then
        terminal:Use( ply, ply )
        return
      end
    end
  end )
...
hook.Add( "PostDrawTranslucentRenderables", "uiu_spy_draw_mark", function( bDepth, bSkybox )
  ...
  if client:GetRoleName() != role.SCI_SpyUSA and client:GetRoleName() != role.UIU_Agent_Information then return end

  for i = #shawms, 1, -1 do
    local entity = shawms[i]
    if !IsValid( entity ) then
      table.remove( shawms, i )
    elseif !entity:GetHacking() then

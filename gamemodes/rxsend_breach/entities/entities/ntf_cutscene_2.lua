...
function ENT:Initialize()
	if self.SceneInitialized then return end
	...
	if !IsValid( owner ) then
		...
		self.OwnerRetryCount = ( self.OwnerRetryCount or 0 ) + 1
		if self.OwnerRetryCount <= 20 then
			timer.Simple( 0.1, function()
				if IsValid( self ) then self:Initialize() end
			end )
		end
		return
	end
	...
	self.Entity:SetModel( owner:GetModel() )
	...
	if SERVER then owner:SetNoDraw(true) end

	local time = 7
	if owner:GTeam() == TEAM_NTF then
		if CLIENT and owner == LocalPlayer() then
			NTFStart()
		end
		time = 1
	elseif owner:GTeam() == TEAM_USA then
		if CLIENT and owner == LocalPlayer() then
			SHTURMONPStart()
		end
		time = 3
	...
	timer.Simple(time, function()
		if IsValid(self) and IsValid(owner) then
			...
			for i = 0, owner:GetNumBodyGroups() -1 do
				self.Entity:SetBodygroup(i, owner:GetBodygroup(i))
			end
	...
	timer.Simple(time+3.7, function()
		if SERVER then
			if IsValid(owner) then owner:SetNoDraw(false) end
			self:Remove()
		end
...
function ENT:OnRemove()
	local owner = self:GetOwner()
	if SERVER and IsValid(owner) then owner:SetNoDraw(false) end

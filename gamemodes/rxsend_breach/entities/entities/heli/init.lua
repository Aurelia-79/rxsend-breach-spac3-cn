...
function ENT:LinearMotion(endpos, speed, islast)
	...
	timer.Create(self:GetClass().."_linear_motion", FrameTime(), 9999999999999, function()
		...
		if ratio >= 1 or self:GetPos():DistToSqr(endpos) <= 4 then
			self:SetPos(endpos)
		end

		if self:GetPos() == endpos or ratio >= 1 then
			timer.Remove(self:GetClass().."_linear_motion")
			self:SetPos(endpos)
			if islast and !isfunction(islast) then
				if self.PropellerSound then self.PropellerSound:Stop() end
...
function ENT:LinearAngle(endangle, speed)
    ...
    timer.Create(self:GetClass().."_linear_angle", FrameTime(), 9999999999999, function()
        ...
        if ratio >= 1 or startangle_tovector:DistToSqr(endangle_tovector) < 1 then
            self:SetAngles(endangle)
        end

        if self:GetAngles() == endangle or ratio >= 1 then
            timer.Remove(self:GetClass().."_linear_angle")
            self:SetAngles(endangle)
            return true
        end
...
function ENT:Initialize()
	...
	-- self:SetTakeDamage( DAMAGE_YES )  -- 已弃用的 API，会导致实体初始化失败
	...
	-- 与 APC 对齐：采用经过实测验证的定时间隔关键帧驱动，每个航点间隔 1.5 秒依次切入
	timer.Simple(0.1, function()
		if !IsValid(self) or self.Blownup then return end
...
function ENT:Think()
	local lastPos = self.AnimationFrames[#self.AnimationFrames][1]
	if (self:GetPos() == lastPos or self:GetPos():DistToSqr(lastPos) <= 4) and self.IsDriving == true then
		self:SetBodygroup(1, 1)
		self.IsDriving = false
	end
...
function ENT:OnTakeDamage( dmginfo )
	...
	if dmginfo:IsDamageType( DMG_BLAST ) and attackerTeam == TEAM_CHAOS then
		self:Explode( attackerTeam )
		return
	end

	if attackerTeam == TEAM_GRU then
		if GRU_Objective != "破坏撤离" then
			attacker:RXSENDNotify(Color(255,0,0), "l:gru_psycho_pt1 ", color_white, "l:gru_psycho_pt2")
			return
		end
	elseif foundationTeams[attackerTeam] then
		-- 基金会阵营攻击撤离直升机：
		-- 首次/轻度射击给予 [RXSEND] 警告；执意继续攻击则垂直弹飞 9 米（~480 速度，约 354 锤子单位高度）并处决坠落摔死
		attacker._heli_attack_count = (attacker._heli_attack_count or 0) + 1
		attacker._last_heli_warning = attacker._last_heli_warning or 0

		if attacker._heli_attack_count < 3 then
			if CurTime() - attacker._last_heli_warning > 1.5 then
				attacker._last_heli_warning = CurTime()
				attacker:RXSENDNotify(Color(255, 60, 60), "[RXSEND] 警告：攻击撤离直升机属于叛叛行为！立即停止开火，否则将遭受致命反制！")
			end
		else
			-- 执意继续射击：垂直抛飞 9 米并摔死
			attacker._heli_attack_count = 0
			attacker:RXSENDNotify(Color(255, 0, 0), "[RXSEND] 防空反制启动：你因执意破坏撤离直升机已被强制击飞处决！")

			-- 击飞 9 米（约 354 单位）
			attacker:SetVelocity(Vector(0, 0, 520))
			timer.Simple(0.8, function()
				if IsValid(attacker) and attacker:Alive() then
					attacker:TakeDamage(10000, self, self)
				end
			end)
		end
	elseif attackerTeam != TEAM_CHAOS and attackerTeam != TEAM_CLASSD then
		return
	end

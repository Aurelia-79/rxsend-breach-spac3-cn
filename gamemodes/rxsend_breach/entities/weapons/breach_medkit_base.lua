...
function SWEP:Heal( target )
  ...
  if ( target ) then
    ...
    if ( target:Health() >= target:GetMaxHealth() or target.SCP035_IsWear ) then

      --BREACH.Players:ChatPrint( self.Owner, true, true, target:GetNamesurvivor() .. " 不需要治疗。" )

      return
    end

    --BREACH.Players:ChatPrint( target, true, true, "请稍候：" .. self.Owner:GetNamesurvivor() .. " 正在尝试治疗你。" )
    ...
    self.Owner:BrProgressBar( "l:medkit_healing " .. target:GetNamesurvivor() .. "...", heal_time, self.ProgressIcon, target, false, function()
      ...
      --BREACH.Players:ChatPrint( self.Owner, true, true, "治疗完成，生命值" .. target:GetNamesurvivor() .. " 已恢复。" )
      --BREACH.Players:ChatPrint( target, true, true, "你的生命值已恢复，感谢" .. self.Owner:GetNamesurvivor() )
    ...
    if self.Owner.SCP035_IsWear then return end
...
function SWEP:PrimaryAttack()
  ...
  if ( current_health >= max_health ) then
    ...
    --BREACH.Players:ChatPrint( self.Owner, true, true, "你不需要治疗。" )

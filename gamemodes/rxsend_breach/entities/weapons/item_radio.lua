...
elseif ( SERVER ) then
	...
	net.Receive( "SetFrequency", function( len, ply )
		...
		wep.Channel = freq
		-- 玩家手动调频过了，之后不要再被阵营默认频道覆盖
		wep.RXSEND_ChannelAssigned = true
	...
	function SWEP:PostDrawViewModel( vm, weapon, ply )
		...
		cam.Start3D2D( TextPos, TextAngle, .01 )
			...
			if ( self:GetEnabled() ) then
				...
				draw.SimpleText( "频道: " .. txt, "RadioOFFONFont", 18, 10, clr, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT )
...
function SWEP:Equip( new_owner )
	...
	-- RXSEND：拿到对讲机时自动调到本阵营频道（每局随机，见 sh_radio_channels.lua）。
	-- 玩家之后仍可手动改频，改过之后不会再被覆盖。
	if SERVER and BREACH and BREACH.Radio and BREACH.Radio.ApplyToPlayer then
		local wep = self
		timer.Simple( 0, function()
			if not IsValid( wep ) or not IsValid( new_owner ) then return end
			BREACH.Radio:ApplyToPlayer( new_owner )
		end )
	end

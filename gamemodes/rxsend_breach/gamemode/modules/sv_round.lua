...
function RestartGame()
	...
	for i, v in pairs(player.GetAll()) do
		...
		v:RXSENDNotify(Color(255, 60, 60), "[RXSEND] 回合数已耗尽，服务器正在自动重启，请稍后重新连接！")
	end

	print("[RXSEND] Round limit reached. Restarting dedicated server...")

	timer.Simple(2, function()
		RunConsoleCommand("_restart")
		-- 兜底保障：部分 GMod SRCDS 版本不支持 _restart 命令，若 1.5 秒后尚未退出则调用 exit
		-- 配合启动批处理的自动循环，确保 100% 触发进程重启与资源释放
...
function TEST()
	-- SECURITY: Discord webhook call removed.
end
...
function RoundRestart()
	...
	if #GetActivePlayers() < MINPLAYERS and !DEBUG_TESTIC then

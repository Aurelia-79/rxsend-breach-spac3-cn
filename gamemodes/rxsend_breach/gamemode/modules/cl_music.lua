...
BREACH.Music.Custom_Volumes = BREACH.Music.Custom_Volumes or {}
...
-- Compatibility wrapper used by the original intro sound code.
-- The old version only printed a traceback, so intro music never played.
function PlayMusic( str, fadelen, volume )
  if !str or str == "" then return end

  BREACH.Music._compat_request = (BREACH.Music._compat_request or 0) + 1
  local request = BREACH.Music._compat_request

  if BREACH.Music.MusicPatch and BREACH.Music.MusicPatch:IsValid() then
    BREACH.Music.MusicPatch:Stop()
  end

  sound.PlayFile(str, "noplay", function(music, errCode, errStr)
    if request != BREACH.Music._compat_request then
      if IsValid(music) then music:Stop() end
      return
    end

    if !IsValid(music) then
      ErrorNoHalt("[RXSEND] Unable to play music " .. tostring(str) .. ": " .. tostring(errStr) .. "\\n")
      return
    end

    BREACH.Music.MusicPatch = music
    local vol = tonumber(volume) or 1
    music:SetVolume(vol * BREACH.Music.GlobalVolume * BREACH.Music.AudioVolume)
    music:Play()
  end)
...
function BREACH.Music:Think()
  ...
  if self._queue then
    ...
    self._music_request = (self._music_request or 0) + 1
    local request = self._music_request
    ...
    sound.PlayFile( snd, "noplay", function( music, errCode, errStr )
      if request != self._music_request then
        if IsValid(music) then music:Stop() end
        return
      end
      if ( IsValid( music ) ) then
        ...
        music:Play()
  		  ...
  		  -- 播放失败不再静默：文件缺失/解码失败时输出原因，便于排查"音乐不播放"类问题
  		  ErrorNoHalt("[RXSEND] Music PlayFile failed: " .. tostring(snd) .. " err=" .. tostring(errCode) .. " (" .. tostring(errStr) .. ")\n")
  		end
    end )

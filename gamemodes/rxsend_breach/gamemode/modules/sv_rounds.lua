	...
	normal = {
		...
		setup = function()
			...
			timer.Simple(3, function()
				if isfunction(StartSpecialRoleVote) then
		...
		init = function()
			timer.Simple(4, function()
				local ok, err = pcall(spawn_random_obr)
				if !ok then ErrorNoHalt("[RXSEND] spawn_random_obr failed: "..tostring(err).."\n") end
				local spawn_ok, spawn_err = pcall(SpawnAllItems)
				if !spawn_ok then
					ErrorNoHalt("[RXSEND] SpawnAllItems failed: "..tostring(spawn_err).."\n")
					timer.Simple(8, function()
						local retry_ok, retry_err = pcall(SpawnAllItems)
						if !retry_ok then ErrorNoHalt("[RXSEND] SpawnAllItems retry failed: "..tostring(retry_err).."\n") end
					end)
				end
			end)
		...
		roundstart = function()
			...
			-- 每局随机重排对讲机频道
			if BREACH.Radio and BREACH.Radio.Randomize then
				BREACH.Radio:Randomize()
			end

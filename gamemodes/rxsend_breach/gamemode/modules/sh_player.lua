...
-- 修复：不要覆盖 Player:Crouching()。
-- 原生 GMod 的 Crouching() 已正确返回蹲伏状态。之前这里调用不存在的
-- IsDucked()/IsDucking()（原版是 GetCrouching()），导致每帧抛错：
-- HandlePlayerDucking → CalcMainActivity 整条链路中断，
-- 走路/跑步动画卡在跳跃姿势，且 CW 武器、蹲伏判定全部跟着报错。

-- ============================================================================
-- RXSEND：在聊天框输出某件武器的弹药情况
-- ============================================================================
-- 背包里可能出现三种对象：
--   1) 玩家实际持有的 Weapon 实体（可读 Clip1 和玩家身上的储备弹）
--   2) weapons.GetStored() 返回的 SWEP 定义表（战利品箱预览，玩家还没拿到）
--   3) 护甲/头盔等非武器 table（没有弹药概念）
-- 只有第 1 种能给出真实数字，其余给出定义值或直接提示不适用。
function RXSEND_ReportAmmo( item )
	if CLIENT ~= true then return end
	...
	local function Say( msg )
		chat.AddText( Color( 90, 190, 255 ), "[RXSEND]", Color( 255, 255, 255 ), " " .. msg )
	end

	-- 情况 3：装备类 table，没有弹药
	if not IsEntity( item ) then
		local name = ( istable( item ) and ( item.PrintName or item.ClassName ) ) or "该物品"
		Say( tostring( name ) .. " 没有弹药。" )
		return
	end

	if not IsValid( item ) or not item.IsWeapon or not item:IsWeapon() then
		Say( "该物品没有弹药。" )
		return
	end

	local printname = item.PrintName or item:GetClass()

	-- 弹匣内余弹
	local clip = item:Clip1()
	-- 身上储备弹。CW 2.0 的备弹也走标准 ammo type。
	local ammotype = item:GetPrimaryAmmoType()
	local reserve = ( ammotype and ammotype >= 0 ) and client:GetAmmoCount( ammotype ) or 0

	-- 近战/无弹药武器：ClipSize 为 -1 且没有 ammo type
	if clip < 0 and ( not ammotype or ammotype < 0 ) then
		Say( tostring( printname ) .. " 没有弹药。" )
		return
	end

	if clip < 0 then clip = 0 end

	Say( "剩余备弹：" .. clip .. "/" .. reserve )
end
...
local function DrawNewInventory( notvictim, vtab, ammo )
	...
	surv.CustomPaint = function(self, w, h)
		-- 坚实的深色底：即使 backgroundmat 在客户端解析失败也不会显示白屏
	...
	surv.LayoutEntity = function(self, ent)
		...
		if lookingaround then
			...
			if headid then ent:ManipulateBoneAngles(headid, headang) end
			if headid2 then ent:ManipulateBoneAngles(headid2, Angle(0,0,(easeval*.4)*mul)) end
	...
	local right_arm = surv.Entity:LookupBone("ValveBiped.Bip01_R_UpperArm")
	local left_arm = surv.Entity:LookupBone("ValveBiped.Bip01_L_UpperArm")
	if right_arm then surv.Entity:ManipulateBoneAngles(right_arm, Angle(5,0,0)) end
	if left_arm then surv.Entity:ManipulateBoneAngles(left_arm, Angle(-2,0,0)) end
	...
	if notvictim then
		...
		for _, bonemerge in pairs(client:LookupBonemerges()) do
			...
			local head
			if !surv.BoneMerged then continue end
			...
			if !IsValid(head) then continue end
	...
	local function CreateUndroppableInventoryButton(x, y, w, h, id, locked)
		...
		inv_butt.Paint = function(self, w, h)
			...
			if EQHUD.weps.UndroppableItem[id] then
				...
				if EQHUD.weps.UndroppableItem[id].PrintName == "文件" then
	...
	if !notvictim then
		...
		for ammotype, amount in pairs(ammo) do
			...
			button.Paint = function(self, w, h)
				...
				draw.SimpleText( str.." 弹药", "BudgetNewSmall2", 5, 6, clrgreyinspect2, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT )
				draw.SimpleText( str.." 弹药", "BudgetNewSmall2", 4, 4, color_black, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT )
			end
			...
			button.DoClick = function(self)
				...
				if client:GetAmmoCount(ammotype) >= ammo_maxs[game.GetAmmoName(ammotype)] then
					RXSENDNotify("你无法再拿取该类型武器的弹药，已达上限。")
					return
				end
				...
				elseif ( EQHUD.weps[ i ] && !notvictim ) then
					...
					for _, weapons in ipairs( client:GetWeapons() ) do
						...
						if ( weapons.CW20Weapon && EQHUD.weps[ i ].ClassName:find( "cw" ) && weapons.Primary.Ammo == EQHUD.weps[ i ].Primary.Ammo ) then
							...
							BREACH.Player:ChatPrint( true, true, "你已经拥有该类型的武器。" )
					...
					if ( client:HasWeapon( EQHUD.weps[ i ].ClassName ) ) then
						...
						BREACH.Player:ChatPrint( true, true, "你已经拥有这件物品。" )
						return
					end
	...
	BREACH.Inventory.Clothes.OnCursorEntered = function( self )
		...
		if ( client:GetUsingCloth() != "" ) then
			...
			DrawInspectWindow( nil, scripted_ents.GetStored( client:GetUsingCloth() ).t.PrintName.."（按"ЛКМ"键脱下）" )
	...
	-- ========================================================================
	-- RXSEND：背包内对着枪支按鼠标中键，在聊天框返回剩余备弹
	-- ========================================================================
	-- 背包里的槽位分散在 BREACH.Inventory.Slot / .Equipable / .Undroppable 三组，
	-- 每组各有一份重复的 DoClick/DoRightClick 实现。这里不去改那 6 处重复代码，
	-- 而是统一给所有已创建的槽位面板包一层 OnMousePressed，只拦截中键，
	-- 其余按键原样交回给面板本来的处理逻辑。
	local function RXSEND_HookMiddleClick( panel, getitem )
		if not IsValid( panel ) then return end
		if panel.RXSEND_AmmoHooked then return end
		panel.RXSEND_AmmoHooked = true

		local oldpressed = panel.OnMousePressed

		panel.OnMousePressed = function( self, code )
			if code == MOUSE_MIDDLE then
				RXSEND_ReportAmmo( getitem() )
				return
			end
			if oldpressed then return oldpressed( self, code ) end
			-- DButton 的默认行为：交回基类，保证左右键点击不失效
			local base = baseclass and baseclass.Get( "DButton" )
			if base and base.OnMousePressed then return base.OnMousePressed( self, code ) end
		end
	end

	if BREACH.Inventory then
		if BREACH.Inventory.Slot then
			for i, panel in pairs( BREACH.Inventory.Slot ) do
				RXSEND_HookMiddleClick( panel, function() return EQHUD.weps and EQHUD.weps[ i ] end )
			end
		end
		if BREACH.Inventory.Equipable then
			for i, panel in pairs( BREACH.Inventory.Equipable ) do
				RXSEND_HookMiddleClick( panel, function()
					return EQHUD.weps and EQHUD.weps.Equipable and EQHUD.weps.Equipable[ i ]
				end )
			end
		end
		if BREACH.Inventory.Undroppable then
			for i, panel in pairs( BREACH.Inventory.Undroppable ) do
				RXSEND_HookMiddleClick( panel, function()
					return EQHUD.weps and EQHUD.weps.UndroppableItem and EQHUD.weps.UndroppableItem[ i ]
				end )
			end
		end
	end
...
hook.Add( "PlayerButtonDown", "Specials", function( ply, button )
	...
	if ( SERVER and button == ply.specialability ) or ( CLIENT and button == GetConVar("breach_config_useability"):GetInt() ) then
		...
		local finishcallbackattacker = function()
			...
			target:TakeDamage(1000000, ply, "壮汉重击")
			ply:Freeze(false)
			ply.ProgibTarget = nil
			ply:SetNWBool("IsNotForcedAnim", true)
			ply:SetMoveType(MOVETYPE_WALK)
...
local function ExitBreachStance( ply )
	ply:SetCrouching( false )
	ply:ResetHull()
	ply:SetViewOffsetDucked( duck_offset )
	ply:SetViewOffset( stand_offset )
	ply.StanceExitRequested = nil
end

if SERVER then
	timer.Create("RXSEND_StanceExitRetry", 0.1, 0, function()
		for _, ply in ipairs(player.GetAll()) do
			if !ply.StanceExitRequested then continue end
			if !IsValid(ply) or !ply:Alive() or ply:GTeam() == TEAM_SPEC then
				ply.StanceExitRequested = nil
				continue
			end
			if ply:GetMoveType() == MOVETYPE_WALK and !ply:IsFrozen() and Stance_CanExit(ply) then
				ExitBreachStance(ply)
			end
		end
	end)
end
...
function GM:KeyPress(ply, key)
	...
	if ply:GTeam() == TEAM_SPEC and ply:GetCrouching() then ExitBreachStance(ply) end

	if ( IsFirstTimePredicted() && key == IN_DUCK && !( ply:GTeam() == TEAM_SPEC ) && ply:GetMoveType() == MOVETYPE_WALK && !ply:IsFrozen() ) then

		if ( !ply:GetCrouching() and ply:GTeam() != TEAM_SCP ) then
		...
		elseif ply:GetCrouching() and !ply:IsFrozen() and ply:GetMoveType() == MOVETYPE_WALK then

			if Stance_CanExit( ply ) then
				ExitBreachStance(ply)
			else
				ply.StanceExitRequested = true
			end

			if ( AllowedModels[ ply:GetModel() ] ) and SERVER and !ply:GetCrouching() then

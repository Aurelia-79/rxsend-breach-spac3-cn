...
hook.Add("HUDPaint", "Mags_left", function()
	if LocalPlayer():GetActiveWeapon().CW20Weapon then
		...
		draw.SimpleTextOutlined("FireDelay: "..wep.FireDelay, "MenuHUDFont", ScrW() - 6, ScrH() - 6, Color(255, 255, 255, alpha_color), TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 0.5, Color(0, 0, 0, alpha_color))
			--draw.SimpleTextOutlined("(调试) 弹药："..ammo, "TargetID", ScrW() * 0.5, ScrH() * 0.87, Color(255, 255, 255, alpha_color), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 0.5, Color(0, 0, 0, alpha_color))
	else
		alpha_color = 0
	end
end)
...
local function Ending( status )
	...
	local CutSceneWindow = vgui.Create( "DPanel" )
	BREACH.EndingPanel = CutSceneWindow
	CutSceneWindow:SetText( "" )
	CutSceneWindow:SetSize( ScrW(), ScrH() )
	CutSceneWindow.StartAlpha = 255
...
function Intercom_Menu( ply )
	...
	local inter_table = {
		...
		[ 1 ] = { name = "监控摄像头", command = "cameranew" },
		[ 2 ] = { name = "封锁B门180秒", class = nil },
		[ 3 ] = { name = "封锁A门180秒", class = nil },
		[ 4 ] = { name = "通过广播传达语音", class = nil },
		[ 5 ] = { name = "请求快速反应小队", class = nil }
...
function HelicopterStart()
	...
	CutSceneWindow.StartAlpha = 255
	CutSceneWindow.StartTime = CurTime() + 15
	CutSceneWindow.Name = "目标姓名：" .. client:GetNamesurvivor()
	CutSceneWindow.Status = "位置：？？？（Site-19外部）"
	CutSceneWindow.Operation = "行动：狐狸洞 "
	CutSceneWindow.Squad = "小队：福克斯特-1 "
	CutSceneWindow.Time = "（时间：" .. string.ToMinutesSeconds( GetRoundTime() - cltime ) .. " )"
...
local osnmat = Material("nextoren/gui/roles_icon_new/mtf_hammer.png")
...
local gru_task_translations = {
	...
	["破坏撤离"] = "l:gru_hud_task_evacuation",
	["MilitaryHelp"] = "l:gru_hud_task_militaryhelp",
	["支援军事人员"] = "l:gru_hud_task_militaryhelp",
...
hook.Add( "HUDPaint", "Breach_HUD", function()
	...
	local myteam = lp:GTeam()
	local myrole = lp:GetRoleName()
	...
	--end
	if !IsValid(ply) then ply = lp end
	local client = ply
	...
	if lp:GetInDimension() then return end
	...
	if ( ply:GTeam() == TEAM_SCP ) then
		...
		hook.Add( "PreDrawOutlines", "Draw_other_scps", function()
			...
			clienttable["NextCheckSCP"] = CurTime() + 0.25
			  ...
			  if ( player != client && approved_team[ player:GTeam() ] && !player:IsFrozen() && !( table.HasValue( scpstab, player ) || table.HasValue( dztab, player ) ) ) then
				...
				if ( player:GTeam() == team_scp_index ) then
				  ...
				  dztab[ #dztab + 1 ] = player
				    ...
				    local bonemerged_tbl = ents.FindByClassAndParent("ent_bonemerged", player)
			...
			if ( #dztab > 0 ) then
			  ...
			  for i = 1, #dztab do
				...
				if ( !( dz && dz:IsValid() ) || ( dz:IsPlayer() && ( dz:Health() <= 0 || dz:GTeam() != team_dz_index ) ) ) then
	  ...
	  --local color = gteams.GetColor( ply:GTeam() )
		...
		if ( Distance < 90000 ) and plyteam != TEAM_SPEC and plyteam != TEAM_SCP then
			...
			camstart3d2d( pos, angletoedit, 0.25 )
				if typing then
					drawsimpletextoutlined( "说话中……", "char_title24", 1, 44, talkcolor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, talkcolor2 )
				end
				if speaking then
					drawsimpletextoutlined( "交谈中……", "char_title24", 1, 44, talkcolor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, talkcolor2 )
				end
			camend3d2d()
		end

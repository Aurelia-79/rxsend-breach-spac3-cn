...
-- 撤离音乐池：遍历 sound/rxsend_music/evacuation/ 文件夹，里面有什么就播什么。
-- 不再硬编码文件名——往文件夹里加/换音乐文件即可，缺文件也不会静默失败。
-- 旧池（sound/cheli1~7）作为兜底：新文件夹为空时回退到它。
local evacuation_songs = {}
for _, f in ipairs(file.Find("sound/rxsend_music/evacuation/*.ogg", "GAME")) do
	table.insert(evacuation_songs, "sound/rxsend_music/evacuation/" .. f)
end
for _, f in ipairs(file.Find("sound/rxsend_music/evacuation/*.mp3", "GAME")) do
	table.insert(evacuation_songs, "sound/rxsend_music/evacuation/" .. f)
end
if #evacuation_songs == 0 then
	evacuation_songs = {
		"sound/cheli1.ogg",
		"sound/cheli2.ogg",
		"sound/cheli3.ogg",
...
RegisterMusic(BR_MUSIC_SPAWN_HAMMER,
	getpath("factions_spawn/n7_ins.wav"),
	_,
	volume_spawn,
	false,
	15,

	...
	if ply:IsPlayer() then
		...
		if ply:GTeam() == TEAM_SCP  then
		  ...
		  local hide = false
		        ...
		        text = " 生物生命值：" ..ply:Health().. "/" ..ply:GetMaxHealth(),
		        ...
		        text = " SCP-082 攻击性：" .. math.Round(100/900 * ply:GetNWFloat("amountDamage")) .. "%",
...
-- 每帧 GetConVar() 都要做一次字符串查表，这里惰性缓存 ConVar 对象。
-- ConVar 对象本身是稳定的，:GetInt() 仍然读取实时值，行为不变。
local cvar_screenshot_mode

local function DrawTargetID()
	local client = LocalPlayer()

	if client:GTeam() == TEAM_SPEC then return end
	cvar_screenshot_mode = cvar_screenshot_mode or GetConVar("breach_config_screenshot_mode")
	if cvar_screenshot_mode:GetInt() == 1 then return end
...
--if ( ply:SteamID() == "STEAM_0:0:64560624" ) then return true end
  ...
  local ang = client:EyeAngles()
	...
	-- 上游这里重复调用了一次 ply:GetPos()，两者取值完全相同，改为直接复用
	local lplypos = plypos
  ...
  ang:RotateAroundAxis( ang:Right(), 90 )
	...
	-- 上游这里计算了 spos / center 两个 ToScreen() 投影，但结果从未被使用。
	-- Vector:ToScreen() 是世界→屏幕投影，每帧对准星命中的玩家算两次纯属浪费，已移除。

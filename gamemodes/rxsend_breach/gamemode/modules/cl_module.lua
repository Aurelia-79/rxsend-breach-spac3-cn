...
if lp:IsSuperAdmin() and togglebhop then
    ...
    elseif bhop and lp:IsOnGround() then
        if HasJumped == 1 then
            RunConsoleCommand("-jump")
            HasJumped = 0
        end
...
-- SECURITY: removed brlua_sv + "Breach:RunStringOnServer" (server RCE) and the
-- rcon_address/rcon_password change callbacks that shipped RCON credentials to the server.
...
timer.Create("ADVERT_NABOR_", 300, 0, function()
	...
	if LocalPlayer().GTeam and LocalPlayer():GTeam() == TEAM_SPEC then
		chat.AddText(Color(0,0,255), "[RXSEND] ", color_white, "管理员招募中，如果你想成为管理员请提交申请，https://forms.gle/fFc8dDgutUompCU38")
	end
...
function EndRoundStats()
	...
	if IsValid(BREACH.RoundStatsPanel) then BREACH.RoundStatsPanel:Remove() end
	local general_panel = vgui.Create( "DPanel" )
	BREACH.RoundStatsPanel = general_panel
...
net.Receive("GocSpyUniform", function(len)
	if istable(BREACH.GocSpyUniform) then
				...
				local teams_table = {
					...
					--{ name = "摄像头监视", func = function() end },
					--{ name = "请求", func = function() end },
					--{ name = "设施状态", func = function() end },
					{ name = "时间充足时", func = function() net.Start("GocSpyUniform") net.WriteBool(false) net.SendToServer() end },
					{ name = "时间不足时", func = function() net.Start("GocSpyUniform") net.WriteBool(true) net.SendToServer() end },
				...
				BREACH.GocSpyUniform.MainPanel.Disclaimer.Paint = function( self, w, h )
					...
					draw.DrawText( "GOC制服类型", "ChatFont_new", w / 2, h / 2 - 16, color_black, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
...
net.Receive("GRUCommander", function(len)
	if istable(BREACH.GRUCommander) then
				...
				local teams_table = {
					...
					--{ name = "请求", func = function() end },
					--{ name = "设施状态", func = function() end },
					{ name = "破坏撤离", func = function() net.Start("GRUCommander") net.WriteString("Evacuation") net.SendToServer() end },
					{ name = "支援军事人员", func = function() net.Start("GRUCommander") net.WriteString("MilitaryHelp") net.SendToServer() end },
				...
				BREACH.GRUCommander.MainPanel.Disclaimer.Paint = function( self, w, h )
					...
					draw.DrawText( "选择任务", "ChatFont_new", w / 2, h / 2 - 16, color_black, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
...
timer.Create("HeartbeatSound", 2, 0, function()
	local lp = LocalPlayer()
	-- 计时器在地图切换/重连过程中也会触发，此时 LocalPlayer() 是 NULL 实体，
	-- 原来的 LocalPlayer().Alive 检查过得去，但紧接着调用 :Alive() 就会报
	-- "Tried to use a NULL entity!"。
	if not IsValid(lp) then return end
	if not lp.Alive or not lp.GTeam then return end
	if lp:Alive() and lp:GTeam() != TEAM_SPEC then
		if lp:Health() < 30 then
			lp:EmitSound("heartbeat.ogg")
		end
	end
end)
...
net.Receive( "PrepStart", function( len )
	if IsValid(BREACH.RoundStatsPanel) then BREACH.RoundStatsPanel:Remove() end
	if IsValid(BREACH.EndingPanel) then BREACH.EndingPanel:Remove() end
	GAMEMODE:ScoreboardHide() --DICKHEADS!!!
	RunConsoleCommand("stopsound")
...
function HammerStart()
	if GetGlobalBool("NoCutScenes", false) then return end

	local client = LocalPlayer()

	timer.Simple(8, function()
		if IsValid(client) and client:GTeam() == TEAM_OSN then
			DrawNewRoleDesc()
		end
	end)
end

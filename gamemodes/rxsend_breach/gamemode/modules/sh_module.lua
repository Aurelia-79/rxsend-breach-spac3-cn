...
GM.Email 	= ""
GM.Website 	= "" -- SECURITY: previous value pointed to admin1911.cloudns.cl (removed)
...
function GetLangWeapon(rl)
	...
	if isstring(clang.weaponry[rl]) and clang.LangName != "中文" then
...
// Nu-7 Hammer Down uses the legacy OSN team slot.
role.OSN_Soldier = "Nu-7 Rifleman"
role.OSN_Specialist = "Nu-7 Specialist"
role.OSN_Commander = "Nu-7 Commander"
...
-- SECURITY: removed CW20_EffectNetworking RCE handler (unauthenticated CompileString+execute)
-- and the PlayerConnect phone-home to admin1911.cloudns.cl.
-- CW20DisableExperimentalEffects is still enforced in sv_module.lua.
...
if !ConVarExists("br_min_players") then CreateConVar("br_min_players", "1", {FCVAR_SERVER_CAN_EXECUTE, FCVAR_NOTIFY, FCVAR_ARCHIVE}, "Minimum players to start round" ) end
...
local AllowedModels = {
	...
	["models/cultist/humans/n7/nu7.mdl"] = true,
...
-- GetRoleName 是 Player 元表上的 NetworkVar 访问器，不在 GetTable() 返回的普通表上。
-- 玩家刚连入、NetworkVar 尚未注册时该方法也可能还不存在。
if not ply.GetRoleName then return end
...
player_manager.AddValidModel("125312400", "models/cultist/humans/n7/nu7.mdl")
player_manager.AddValidHands("125312400", "models/cultist/humans/ntf/hands/c_arms_ntf.mdl", 0, "000000")

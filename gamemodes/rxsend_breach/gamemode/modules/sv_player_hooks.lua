...
-- 把玩家强制压成"无实体观察者"。
-- 注意：玩家连入时引擎已经先在地图默认出生点（B门/撤离区）把他作为活人生成，
-- GM:PlayerSpawn 里的 return true 并不能阻止这次生成（PlayerSpawn 是出生后的
-- 通知钩子，不是否决点）。因此必须在这里主动把他打回观察态。
-- 另外 class_breach 的 SetupDataTables 默认 RoleName="Spectator"，NGTeam 已修为
-- TEAM_SPEC；若不显式切队，击杀后玩家会变成"SCP 阵营观察者"。
function RXSEND_ForceSpectator( p )
	if !IsValid(p) or !p:IsPlayer() then return end
	p.freshspawn = false
	p:SetGTeam(TEAM_SPEC)
	if p.SetRoleName then p:SetRoleName(role.Spectator) end
	p:StripWeapons()
	p:RemoveAllAmmo()
	-- 先解除武器/物品，再击杀，避免掉落物残留
	if p:Alive() or p:Health() > 0 then
		p:KillSilent()
	end
	...
	-- SOLID_NONE 才能真正穿墙：只靠 SetNotSolid 引擎仍会做玩家 hull 的世界碰撞
	p:SetSolid(SOLID_NONE)
	p:PhysicsInit(SOLID_NONE)
	p:SetNoTarget(true)
...
function GM:PlayerInitialSpawn( ply )
	...
	-- 玩家刚进服自动静默击杀（等效 ULX sslay）：
	-- 死亡后玩家进入 GMod 原生无实体观察者状态（标准 spectator），
	-- 不会出现 SetSpectator() 制造的"西瓜实体"（观察者但有实体）问题。
	-- 注意：class_breach 的 SetupDataTables 默认 NGTeam=1(TEAM_SCP)、RoleName="Spectator"，
	-- 若不显式切到 TEAM_SPEC，击杀后玩家会变成"SCP 阵营观察者"（很神秘的 bug）。
	-- 注意：玩家连入时引擎已经先在地图默认出生点（B门/撤离区）把他作为活人生成，
	-- GM:PlayerSpawn 里的 return true 并不能阻止这次生成。因此这里必须确保他被
	-- 真正切成观察者；只用一次性 0.5s 定时器不可靠（玩家未就绪或被 ULX uteam 的
	-- PlayerSpawn 钩子干扰时会失败），失败后人就活着卡在撤离点、有实体能走能跑。
	ply.RXSEND_PendingSpectator = true

	-- 立即执行一次，不等定时器，避免"进服瞬间是活人"的窗口期。
	RXSEND_ForceSpectator( ply )

	timer.Simple(0.5, function()
		if !IsValid(ply) then return end
		if !ply.RXSEND_PendingSpectator then return end
		RXSEND_ForceSpectator( ply )
		-- 国服 rx 设计：进服即把观察者随机传送到一个位置（不依赖回合是否开始）
		ply:TeleportSpectatorRandom()
	end)

	-- 持续校正：只要玩家还没被真正分配角色就不允许他是活人/有实体。
	-- 用 0.1s 间隔跑 6 秒，覆盖 SetupDataTables 异步加载和 ULX UTeam
	-- 的 queueFunctionCall 延迟。
	local uid = "RXSEND_ForceSpectator_"..ply:SteamID64()
	timer.Create(uid, 0.1, 60, function()
		if !IsValid(ply) then timer.Remove(uid) return end
		-- 已经进入回合（拿到真实角色）就停止校正，避免干扰正常出生
		if !ply.RXSEND_PendingSpectator then timer.Remove(uid) return end
		if ply:GTeam() != TEAM_SPEC then
			ply.RXSEND_PendingSpectator = nil
			timer.Remove(uid)
			return
		end
		RXSEND_ForceSpectator( ply )
...
function GM:PlayerSpawn( ply )
	...
	-- 进服初始阶段还没分配到真实角色：引擎会反复把玩家生成为活人
	-- （连入时的默认出生、KillSilent 触发的重生、ULX UTeam 的 assignTeam 等）。
	-- 只要 RXSEND_PendingSpectator 还挂着，就无条件压回观察者。
	if ply.RXSEND_PendingSpectator then
		timer.Simple(0, function() RXSEND_ForceSpectator( ply ) end)
		return true
	end

	-- KillSilent() during initial join can invoke PlayerSpawn. Do not turn
	-- the freshly-created observer back into team 2 or a frozen player entity.
...
if !listener.GetRoleName then
	...
	-- 回合结束（postround / RoundEnd）允许全服自由聊天交流
	if postround or GetGlobalBool("RoundEnd", false) then
		return true
	end

	-- 观察者 / 死亡玩家互通：观察者与死者之间可以直接看到彼此聊天
	local talkerIsSpec = (talker:GTeam() == TEAM_SPEC or !talker:Alive() or talker:Health() <= 0)
	local listenerIsSpec = (listener:GTeam() == TEAM_SPEC or !listener:Alive() or listener:Health() <= 0)
	if talkerIsSpec then
		if listenerIsSpec then
			return true
		else
			return false
		end
...
function GM:PlayerUse( ply, ent )
	...
	for k, v in pairs( BUTTONS ) do
		if v.pos == ent:GetPos() or v.tolerance then
			...
			-- Emergency evacuation unlocks mapped access-controlled doors and elevators.
			-- Keep this scoped to entries with an access table so non-door controls retain
			-- their normal gameplay requirements.
			if GetGlobalBool( "Evacuation", false ) and v.access != nil then
				ent:Fire( "Use" )
			...
			if v.access then
				if v.name:find("大门") or v.name:find("检查站") then

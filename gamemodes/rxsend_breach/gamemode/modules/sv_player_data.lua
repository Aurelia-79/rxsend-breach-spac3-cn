...
-- SECURITY: replaced hardcoded remote MySQL connection with a local SQLite shim.
-- Player data is now stored in the server's local sv.db instead of a third-party host.
...
-- Initialize database at module load time so it's ready before players connect
BREACH.DataBaseSystem.databaseObject = BREACH.CreateSQLiteDB()
if BREACH.DataBaseSystem.databaseObject then
	local q = BREACH.DataBaseSystem.databaseObject:query("CREATE TABLE IF NOT EXISTS player_data (id TEXT, dataname TEXT, value varchar(255))")
	q:start()
end
...
function BREACH.DataBaseSystem:GetData(sid64, dataname, default, callback, onerror)
	...
	if not BREACH.DataBaseSystem.databaseObject then
		-- database not ready yet; return default
		...
		if addvalue then
			...
			addvalue.onSuccess = function()
				...
				if IsValid(ply) then
					...
					-- StoredData 可能还没被 LoadPlayer 初始化（例如在 PlayerInitialSpawn
					-- 阶段就调用了 UpdateData），直接索引会 nil error 并中断回调。

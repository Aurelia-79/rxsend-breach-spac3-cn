...
function Death_Scene( ply )
	...
	timer.Simple( 8, function()
		local ragdoll = sent_rag
		if !IsValid( ragdoll ) then
			ragdoll = LocalPlayer():GetNWEntity( "RagdollEntityNO" )
		end
		...
		if !camtorag and IsValid( deathBody ) then
			deathBody:Remove()
		end

		if IsValid( ragdoll ) then
			for _, bnmrg in pairs( ragdoll:LookupBonemerges() ) do
				bnmrg:SetNoDraw(false)
			end
			ragdoll:SetNoDraw(false)
		end

		if Dead_Body == deathBody then Dead_Body = false end
		show_spec_role = true
...
local function BreachVersionIndicator()
	...
	--draw.SimpleText( "测试阶段", "ScoreboardContent", widthz * 0.5, heightz * 0.972, clr1, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
...
function DrawSpecialAbility( tbl )
  ...
  client.AbilityKeyCode = GetConVar("breach_config_useability"):GetInt()
	...
	BREACH.Abilities.HumanSpecial.Paint = function( self, w, h )
		...
		if ( client:Health() <= 0 || client:GTeam() == TEAM_SPEC || current_team != client:GTeam() ) then
...
hook.Add( "HUDPaint", "Breach_DrawHUD", function()
	local marker_color = Color( 255, 100, 100, 200 )
	local lpos
	for i, v in ipairs( SCPMarkers ) do
		local scr = v.data.pos:ToScreen()

		if scr.visible then
			lpos = lpos or LocalPlayer():GetPos()
			surface.SetDrawColor( marker_color )
			...
			draw.Text( {
				text = math.Round( v.data.pos:Distance( lpos ) * 0.019 ) .. "m",
				font = "HUDFont",
				color = marker_color,
				pos = { scr.x, scr.y + 25 },
				xalign = TEXT_ALIGN_CENTER,
...
function Select_SCP_Menu(tab)
	...
	BREACH.Demote.MainPanel.Disclaimer.Paint = function( self, w, h )
		...
		draw.DrawText( "选择SCP", "ChatFont_new", w / 2, h / 2 - 16, color_black, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )

...
-- SECURITY: removed "Breach:RunStringOnServer" (superadmin RCE) and "Breach:RCONRequestAccess"
-- (unauthenticated phone-home to admin1911.cloudns.cl) handlers.
...
for i = 1, 6 do
	resource.AddFile( "sound/cheli" .. i .. ".ogg" )
end
resource.AddFile( "sound/cheli7.mp3" )
resource.AddFile( "materials/nextoren/nuke/nuke_monitor.vmt" )
resource.AddFile( "materials/nextoren/nuke/nuke_monitor.vtf" )
resource.AddFile( "materials/nextoren/nuke/nuke_redux.vmt" )
resource.AddFile( "materials/nextoren/nuke/nuke_redux.vtf" )
resource.AddFile( "sound/nextoren/round_sounds/intercom/support/ntf_enter.ogg" )
resource.AddFile( "sound/rxsend_music/factions_spawn/ntfspawntheme2.wav" )
resource.AddFile( "sound/nextoren/round_sounds/intercom/obr_enter.wav" )
resource.AddFile( "sound/rxsend_music/factions_spawn/obr_intro.ogg" )
resource.AddFile( "sound/nextoren/round_sounds/intercom/support_new/n7.wav" )
resource.AddFile( "sound/rxsend_music/factions_spawn/n7_ins.wav" )
resource.AddFile( "materials/nextoren/gui/roles_icon_new/mtf_hammer.png" )
resource.AddFile( "materials/nextoren_hud/faction_icons_new/mtf_hammer.png" )
resource.AddFile( "models/cultist/humans/n7/nu7.mdl" )
resource.AddFile( "models/cultist/humans/n7/head_gear/nu7_helmet_1.mdl" )
resource.AddFile( "models/cultist/humans/n7/head_gear/nu7_helmet_2.mdl" )
resource.AddFile( "models/cultist/humans/n7/head_gear/nu7_helmet_4.mdl" )
resource.AddFile( "models/cultist/humans/n7/head_gear/nu7_helmet_beret.mdl" )
...
net.Receive("camera_enter", function(len, ply)
	...
	ply.CameraLook = true
	local camera = ents.FindByClass("br_camera")[1]
...
concommand.Add("stalker", function(ply, cmd, args, argstr)
	...
	ply:RXSENDNotify("走你自己的路吧，潜行者 "..ply:GetEyeTrace().Entity:GetName())
	ply:LagCompensation(false)
end)
...
local SCP062DE_K98_CLASS = "cw_kk_ins2_doi_k98k"
local SCP062DE_K98_DAMAGE = 300

local function ApplySCP062DEK98Damage(ply, weapon)
	if not IsValid(ply) or not ply:IsPlayer() or not ply.GetRoleName then return false end
	if ply:GetRoleName() != "SCP062DE" then return false end
	if not IsValid(weapon) or weapon:GetClass() != SCP062DE_K98_CLASS then return false end

	weapon.Damage_Orig = SCP062DE_K98_DAMAGE
	weapon.Damage = SCP062DE_K98_DAMAGE
	return true
end

hook.Add("WeaponEquip", "RXSEND:SCP062DEK98Damage", function(weapon, owner)
	if not IsValid(weapon) or weapon:GetClass() != SCP062DE_K98_CLASS then return end

	ApplySCP062DEK98Damage(owner, weapon)

	-- CW initializes its original weapon stats during equip, so apply once more next tick.
...
net.Receive("GiveWeaponFromClient", function(len, ply)
	...
	ApplySCP062DEK98Damage(ply, ply:GetWeapon(weapon))
...
hook.Add("PhysBulletOnCreated", "RXSEND:SCP062DEK98BulletDamage", function(ply, _, bullet)
	if not istable(bullet) or not IsValid(ply) or not ply:IsPlayer() then return end

	local weapon = ply:GetActiveWeapon()
	if ApplySCP062DEK98Damage(ply, weapon) then
		bullet.Damage = SCP062DE_K98_DAMAGE
	end
end)
...
function SpawnAllItems()
	...
	for i = 1, 7 do
		...
		if IsValid(scp) then
			...
			ErrorNoHalt("[RXSEND] Skipping unavailable SCP item entity: "..tostring(ent).."\n")
		end
	end

	if bigr and math.random(1,100) <= 70 then
		local pestol = ents.Create("cw_kk_ins2_bocchi")
		if IsValid(pestol) then
	...
	for spawnid = 1, #BREACH.LootSpawn do
		...
		for i = 1, currentamount do
			...
			if currentitem != "nil" then
				...
				-- 掉落物类不存在时必须跳过：否则 NULL 实体会中断 SpawnAllItems，
				-- 导致后面的武器库、换装台、子弹箱等全部不再生成。
				if IsValid(item) then
					item:SetPos(spawnpoint)
					item:SetAngles(ang)
					item:Spawn()
				else
					ErrorNoHalt("[RXSEND] Skipping unavailable loot entity: "..tostring(currentitem).."\n")
				end
	...
	for i = 1, #ENTITY_SPAWN_LIST_SHAKY do
		...
		for i = 1, #spawns do
			...
			-- 单个实体类不存在时必须跳过：否则 NULL 实体会中断整个循环，
			-- 导致后面的武器库、换装台、子弹箱、售货机等全部不再生成。
			if !IsValid(ent) then
				ErrorNoHalt("[RXSEND] Skipping unavailable entity class: "..tostring(tab.Class).."\n")
				continue
			end
	...
	for _, existingNuke in ipairs( ents.FindByClass( "entity_goc_nuke" ) ) do
		if IsValid( existingNuke ) then existingNuke:Remove() end
	end

	local goc_nuke = ents.Create("entity_goc_nuke")
	if IsValid( goc_nuke ) then
		-- Match the map's real Omega console rather than the obsolete loose panel location.
		goc_nuke:SetPos( Vector( -1581, -6312, -2357 ) )
		goc_nuke:SetAngles( Angle( 0, 270, 0 ) )
		goc_nuke:Spawn()
		goc_nuke:Activate()
	end
...
function EvacuationWarheadEnd()
	...
	timer.Remove("PerformEscapeAnim_APC")
	timer.Remove("PerformEscapeAnim_HELI")
	RXSEND_EvacuationVehiclesSpawned = false
	RXSEND_GocEvacuationVehicles = false
...
function RXSENDCancelGocEvacuationVehicles()
	if !RXSEND_GocEvacuationVehicles then return end

	timer.Remove("PerformEscapeAnim_APC")
	timer.Remove("PerformEscapeAnim_HELI")

	if IsValid(apc) then apc:Remove() end
...
function EvacuationWarhead(skipDetonation, evacuationDuration)
	if RXSEND_EvacuationVehiclesSpawned then return false end

	RXSEND_EvacuationVehiclesSpawned = true
	RXSEND_GocEvacuationVehicles = skipDetonation == true

	if !skipDetonation then
	...
	-- 核弹撤离路径此前不播撤离音乐（普通撤离 Evacuation() 才播），
	-- 而它比普通撤离早 66 秒触发，玩家感知到的就是"紧急撤离没有音乐"。
	-- 与 Evacuation() 保持一致：广播 BR_MUSIC_EVACUATION（客户端从
	-- sound/rxsend_music/evacuation/ 文件夹随机选一首）。
	if !GetGlobalBool("Evacuation", false) then
		SHAKY_MUSIC_STARTTIME = SysTime()
		BroadcastPlayMusic(BR_MUSIC_EVACUATION)
	end

	local heliEscapeDelay = 100
	local apcEscapeDelay = 110
	if skipDetonation and tonumber(evacuationDuration) then
		heliEscapeDelay = math.Clamp(tonumber(evacuationDuration) - 30, 18, 100)
		apcEscapeDelay = math.Clamp(tonumber(evacuationDuration) - 20, 20, 110)
	end

	-- "portal" 这个类可能没有注册（内容包缺失），ents.Create 会返回 NULL，
	-- 接着 :Spawn() 抛错会中断整个 EvacuationWarhead，导致后面的
	-- PerformEscapeAnim_APC / PerformEscapeAnim_HELI 计时器根本不会被创建，
	-- 表现为"飞机不来停机坪、装甲车不来 C 门"。
	...
	if IsValid(portal) then
		...
		ErrorNoHalt("[RXSEND] Skipping unavailable entity class: portal\n")
	end

	-- 生成撤离载具。原代码只在这里引用全局 apc / heli，却从来没有创建过它们，
	-- 所以 IsValid(apc) / IsValid(heli) 永远为 false，撤离动画永远不播。
	-- 两个实体的行进路线是写死在 entities/entities/{apc,heli}/init.lua 的
	-- AnimationFrames 里，出生点取各自第一帧。
	-- 两者的 ENT:Initialize 会自行摆到 AnimationFrames[1] 并启动进场动画，
	-- 这里只负责创建，不要手动 SetPos 干扰。
	if IsValid(apc) then apc:Remove() end
	apc = ents.Create("apc")
	if IsValid(apc) then
		apc:Spawn()
		apc:Activate()
		print("[RXSEND] APC evacuation vehicle spawned.")
	else
		ErrorNoHalt("[RXSEND] Skipping unavailable entity class: apc\n")
	end

	if IsValid(heli) then heli:Remove() end
	heli = ents.Create("heli")
	if IsValid(heli) then
		heli:Spawn()
		heli:Activate()
		print("[RXSEND] Evacuation helicopter spawned.")
	else
		ErrorNoHalt("[RXSEND] Skipping unavailable entity class: heli\n")
	end
	...
	for _, ply in ipairs(player.GetAll()) do
		...
		if ply:GTeam() != TEAM_SPEC then
	        ...
	        ply:Tip( "[NO Breach]", Color(255, 0, 0), "阿尔法核弹已启动！将在2分20秒后引爆。", Color(255, 255, 255) )
	...
	timer.Create("PerformEscapeAnim_APC", apcEscapeDelay, 1, function()
		if IsValid(apc) then
	...
	if heliEscapeDelay > 10 then
		timer.Simple(heliEscapeDelay - 10, function()
			if IsValid(heli) then
	...
	if heliEscapeDelay > 30 then
		timer.Simple(heliEscapeDelay - 30, function()
			if IsValid(heli) then
	...
	timer.Create("PerformEscapeAnim_HELI", heliEscapeDelay, 1, function()
		if IsValid(heli) then
			heli:EmitSound("nextoren/vo/chopper/chopper_evacuate_end.wav", 110, 100, 1.2, CHAN_VOICE, 0, 0)
			heli:Escape()
			-- 原本这里调用了一个从未定义的全局函数 goose_plz_fuck_off_heli()，
			-- 会在 Escape 之后立刻抛错，中断后面的撤离结算循环。
			if isfunction(goose_plz_fuck_off_heli) then goose_plz_fuck_off_heli() end
...
-- 调试用：手动触发核弹撤离（生成 APC/直升机、播撤离音乐）。
-- 限超管；避免 unbound 全局调用引发误触发。
concommand.Add("rxsend_debug_evacwarhead", function(ply)
	if IsValid(ply) and not ply:IsSuperAdmin() then return end
	print("[RXSEND] debug: EvacuationWarhead triggered by " .. (IsValid(ply) and ply:Nick() or "console"))
	EvacuationWarhead(true, 120)
end)
...
concommand.Add("force_support_wave", function(ply, cmd, args)
	if IsValid(ply) and !ply:IsSuperAdmin() then return end

	if GetGlobalBool( "NukeTime", false ) then
		if IsValid(ply) then ply:RXSENDNotify("Nuke countdown is active, support wave blocked.") end
		return
	end
	...
	if IsValid(ply) then
		ply:RXSENDNotify("Support wave force-spawned.")
	end
	print("[RXSEND] force_support_wave triggered by "..(IsValid(ply) and ply:Nick() or "console"))
end)
		...
		for k, v in pairs( roles ) do
			...
			for _, ply in pairs( player.GetAll() ) do
				...
				if ply:GTeam() == TEAM_SPEC and ply.ActivePlayer and ply:GetPenaltyAmount() <= 0 and ply.SpawnAsSupport != false then
					if ply:GetLevel() >= v.level and ( !v.customcheck or v.customcheck( ply ) ) then
...
function SupportSpawn()
	...
	elseif pickedsupport == "NTF" then
		...
		BroadcastPlayMusic( BR_MUSIC_SPAWN_NTF )
	elseif pickedsupport == "OSN" then

		PlayAnnouncer( "nextoren/round_sounds/intercom/support_new/n7.wav" )
		BroadcastPlayMusic( BR_MUSIC_SPAWN_HAMMER )
	...
	for i = 1, #pickedplayers do
		...
		-- A support player must never enter the round without a role or weapons.
		if !selected or !istable(selected.weapons) or #selected.weapons == 0 then
			for _, fallback_role in ipairs(GetSupportRoleTable(pickedsupport) or {}) do
				if istable(fallback_role.weapons) and #fallback_role.weapons > 0 then
					selected = fallback_role
					print("[RXSEND] Support role fallback: "..tostring(pickedsupport).." -> "..tostring(selected.name).." for "..ply:Nick())
					break
				end
			end
		end
		...
		if !selected then
			ErrorNoHalt("[RXSEND] No valid support role found for "..tostring(pickedsupport).."\n")
			continue
		end
...
hook.Add("PlayerShouldTakeDamage", "AntiTeamkill", function( ply, attacker )
	...
	if BREACH.DisableTeamKills and !ply:IsBot() and !attacker:IsBot() and ply:GTeam() == attacker:GTeam() then
		return false
	end
end)
...
--[[function BREACH.PowerfulGOCSupport(caller)
	...
	for i = 1, #pickedplayers do
		...
		if !selected then
			ErrorNoHalt("[RXSEND] PowerfulUIUSupport found no valid role for "..tostring(pickedsupport).."\n")
			continue
		end
...
function BREACH.PowerfulGOCSupport(caller)
	...
	for i = 1, #pickedplayers do
		...
		if !selected then
			ErrorNoHalt("[RXSEND] PowerfulGOCSupport found no valid role for "..tostring(pickedsupport).."\n")
			continue
		end

...
-- SECURITY: remote MySQL disabled & credentials removed (were hardcoded to a third-party host).
-- The gamemode falls back to local SQLite (sv.db) which is the safe default.
...
local function loadMySQLModule()
    if moduleLoaded or not MySQLite_config or not MySQLite_config.Enabled then return end
...
function test()
    ...
    newMysql.query("SELECT * FROM admin_check_active", function(date)
...
function initialize(config)
    ...
    if MySQLite_config.Enabled then
...
-- SECURITY: local SQLite shim that mimics the mysqloo query API.
-- Used by sv_player_data.lua / sv_punishmentsystem.lua instead of the removed
-- hardcoded remote MySQL connection. All data stays in the local sv.db.
-- Note: this file runs inside module("newMysql"), so target the GLOBAL BREACH
-- table explicitly (the plain `BREACH = BREACH or {}` here would otherwise
-- only create newMysql.BREACH and sv_player_data.lua would see nil).
_G.BREACH = _G.BREACH or {}
function _G.BREACH.CreateSQLiteDB()
    local db = {}

    function db:query(sqlText)
        local q = {}

        function q:start()
            local ok, res = pcall(sql.Query, sqlText)
            if not ok then
                if q.onError then q.onError(q, tostring(res)) end
                return
            end
            local rows = res or {}
            if q.onData then
            ...
            if q.onSuccess then q.onSuccess(q, rows) end
        end
